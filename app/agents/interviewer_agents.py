...
class BaseInterviewer(ABC):
    ...
    @abstractmethod
    async def generate_questions(self, resume_text: str, job_description: str, num_questions: int = 5) -> List[str]:
...
class HRInterviewer(BaseInterviewer):
    ...
    async def generate_questions(self, resume_text: str, job_description: str, num_questions: int = 5) -> List[str]:
        ...
        prompt = f"""
        You are an experienced HR interviewer with a {self.personality} approach.
        Your focus areas are: {', '.join(self.focus_areas)}
        
        Based on the following resume and job description, generate {num_questions} HR interview questions.
        ...
        - Why they're interested in this role and company
        - How they handle workplace challenges
        - Their communication style and team collaboration
        ...
        return await self.gemini_service.generate_questions(
            resume_text, job_description, "hr", num_questions
        )
...
class TechLeadInterviewer(BaseInterviewer):
    ...
    async def generate_questions(self, resume_text: str, job_description: str, num_questions: int = 5) -> List[str]:
        ...
        prompt = f"""
        You are a Senior Technical Lead interviewer with a {self.personality} approach.
        Your focus areas are: {', '.join(self.focus_areas)}
        
        Based on the following resume and job description, generate {num_questions} technical interview questions.
        ...
        Generate questions that assess:
        - Specific technologies mentioned in their resume
        - Problem-solving approach and methodology
        - System design and scalability thinking
        - Code quality and development practices
        - Technical decision-making process
        
        Make questions specific to their experience level and the technologies they've used.
        ...
        return await self.gemini_service.generate_questions(
            resume_text, job_description, "tech_lead", num_questions
        )
...
class BehavioralInterviewer(BaseInterviewer):
    ...
    async def generate_questions(self, resume_text: str, job_description: str, num_questions: int = 5) -> List[str]:
        ...
        return await self.gemini_service.generate_questions(
            resume_text, job_description, "behavioral", num_questions
        )
